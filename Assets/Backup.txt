...
score = 0
fin = open( os.path.join( "Assets", "highscore.txt" ), 'r' )
high_score = fin.read()
fin.close()
...
def isMovable( ):
    if not rightMovable() and not leftMovable() and not upMovable() and not downMovable():
        return False
    else:
        return True
...
def drawTiles( ):
    ...
    for i in range( 4 ):
        for j in range( 4 ):
            if Coordees[i][j][2] == 1:
                tileObj = Tile( Coordees[i][j][0], Coordees[i][j][1], TILE_SIDE, Coordees[i][j][3] )
...
def drawStuff( ):
    ...
    if score > int( high_score ):
        high_score = score
    font = pygame.font.Font( os.path.join( "Assets", "kel.ttf" ), floor( HEIGHT / 9 ) )
        ...
        for j in range( 4 ):
            if Coordees[i][j][2] == 1:
                if Coordees[i - 1][j][2] == 0:
                    ...
                    Coordees[i][j][3] = 0
                elif Coordees[i - 1][j][2] == 1 and Coordees[i][j][3] == Coordees[i - 1][j][3]:
                    if Coordees[i - 1][j][4] == 0 and Coordees[i][j][4] == 0:
                        Coordees[i][j][2] = 0
                        Coordees[i][j][3] = 0
                        Coordees[i - 1][j][3] *= 2
        ...
        for j in range( 4 ):
            if Coordees[-i][j][2] == 1:
                if Coordees[-i + 1][j][2] == 0:
                    ...
                    Coordees[-i + 1][j][3] = Coordees[-i][j][3]
                    Coordees[-i][j][3] = 0
                elif Coordees[-i + 1][j][2] == 1 and Coordees[-i + 1][j][3] == Coordees[-i][j][3]:
        ...
        for j in range( 1, 4 ):
            if Coordees[i][j][2] == 1:
                if Coordees[i][j - 1][2] == 0:
                    ...
                    Coordees[i][j][3] = 0
                elif Coordees[i][j - 1][2] == 1 and Coordees[i][j][3] == Coordees[i][j - 1][3]:
                    if Coordees[i][j - 1][4] == 0 and Coordees[i][j][4] == 0:
                        Coordees[i][j][2] = 0
                        Coordees[i][j][3] = 0
                        Coordees[i][j - 1][3] *= 2
    ...
    while play_Loop:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                fout = open( os.path.join( "Assets", "highscore.txt" ), 'w' )
